...
export default function Home() {
  ...
  function beginCapture() {
    ...
    setPaused(false);
    setElapsed(0);
    notify("Capture started with a visible indicator.");
  }
    ...
    <div className="app-shell" id="notebook-top">
      <aside className={`sidebar ${mobileOpen ? "mobile-open" : ""}`}>
        ...
        <div className="stage-list">
          ...
          <StageRow number="03" label="Media" detail="Next build" />
          <StageRow number="04" label="Live capture" detail="Permission shell" />
        </div>
      ...
      {capturing && <div className="capture-bar"><div className="capture-live"><span className="recording-dot" /> Recording indicator on</div><div className="capture-timer">{formatTimestamp(elapsed)}</div><div className="capture-topic">Current topic <strong>{note.sections[0]?.heading}</strong></div><div className="audio-meter"><Volume2 size={15} /><span><i style={{ width: "64%" }} /></span><small>64%</small></div><button onClick={() => setPaused((value) => !value)}>{paused ? "Resume" : "Pause"}</button><button onClick={saveFrame}><ImageIcon size={14} /> Save frame</button><button className="stop-capture" onClick={stopCapture}>Stop and generate</button></div>}
