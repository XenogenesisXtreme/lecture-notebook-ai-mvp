...
9. While recording, use the in-tab toolbar for **Pause/Resume**, **Save frame**, or **Stop and save** without switching back to the extension popup. The frame is downloaded locally as a JPG. After stopping, review or remove the received WebM in the workspace, then add/import a timestamped transcript to generate grounded notebook sections. The current static build does not yet transcribe raw WebM files automatically.
